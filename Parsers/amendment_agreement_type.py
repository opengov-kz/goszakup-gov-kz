...
def setup_logger(log_filename: str = "amendment_agreement_type.log") -> logging.Logger:
    logger = logging.getLogger("AmendmentAgreementTypeFetcher")
...
class AmendmentAgreementTypeFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_amendment_agreem_type"
    ...
    def get_agreement_types(self) -> list:
        all_types = []
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total amendment agreement types fetched: {len(all_types)}")
        return all_types
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download amendment agreement types from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="amendment_agreement_type.csv", help="Path to output CSV file (default: amendment_agreement_type.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = AmendmentAgreementTypeFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        agreement_types = fetcher.get_agreement_types()
        if agreement_types:
            writer.save_checkpoint(agreement_types)
            writer.write(agreement_types)
        else:
            logger.warning("No agreement types retrieved.")
    except Exception as e:
