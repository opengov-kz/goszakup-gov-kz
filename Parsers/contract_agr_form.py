...
def setup_logger(log_filename: str = "contract_agr_form.log") -> logging.Logger:
    logger = logging.getLogger("ContractAgrFormFetcher")
...
class ContractAgrFormFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_contract_agr_form"
    ...
    def get_contract_agr_forms(self) -> list:
        all_forms = []
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total contract agreement forms fetched: {len(all_forms)}")
        return all_forms
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download contract agreement forms from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="contract_agr_form.csv", help="Path to output CSV file (default: contract_agr_form.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = ContractAgrFormFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        forms = fetcher.get_contract_agr_forms()
        if forms:
            writer.save_checkpoint(forms)
            writer.write(forms)
        else:
            logger.warning("No contract agreement forms retrieved.")
    except Exception as e:
