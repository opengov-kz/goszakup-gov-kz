...
def setup_logger(log_filename: str = "contract_status.log") -> logging.Logger:
    logger = logging.getLogger("ContractStatusFetcher")
...
class ContractStatusFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_contract_status"
    ...
    def get_contract_statuses(self) -> list:
        all_statuses = []
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total contract statuses fetched: {len(all_statuses)}")
        return all_statuses
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download contract statuses from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="contract_status.csv", help="Path to output CSV file (default: contract_status.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = ContractStatusFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        statuses = fetcher.get_contract_statuses()
        if statuses:
            writer.save_checkpoint(statuses)
            writer.write(statuses)
        else:
            logger.warning("No contract statuses retrieved.")
    except Exception as e:
