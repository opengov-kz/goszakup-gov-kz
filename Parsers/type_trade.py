...
def setup_logger(log_filename: str = "type_trade.log") -> logging.Logger:
    logger = logging.getLogger("TypeTradeFetcher")
...
class TypeTradeFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_type_trade"
    ...
    def get_type_trades(self) -> list:
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total type trades fetched: {len(all_types)}")
        return all_types
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download type trades from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="type_trade.csv", help="Path to output CSV file (default: type_trade.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = TypeTradeFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        trades = fetcher.get_type_trades()
        if trades:
            writer.save_checkpoint(trades)
            writer.write(trades)
        else:
            logger.warning("No type trades retrieved.")
    except Exception as e:
