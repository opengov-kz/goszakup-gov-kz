...
def setup_logger(log_filename: str = "contract_cancel.log") -> logging.Logger:
    logger = logging.getLogger("ContractCancelFetcher")
...
class ContractCancelFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_contract_cancel"
    ...
    def get_contract_cancel_reasons(self) -> list:
        all_reasons = []
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total reasons fetched: {len(all_reasons)}")
        return all_reasons
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download contract cancellation reasons from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="contract_cancel.csv", help="Path to output CSV file (default: contract_cancel.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = ContractCancelFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        reasons = fetcher.get_contract_cancel_reasons()
        if reasons:
            writer.save_checkpoint(reasons)
            writer.write(reasons)
        else:
            logger.warning("No cancellation reasons retrieved.")
    except Exception as e:
