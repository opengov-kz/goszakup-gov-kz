...
def setup_logger(log_filename: str = "buy_status.log") -> logging.Logger:
    logger = logging.getLogger("BuyStatusFetcher")
...
class BuyStatusFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_buy_status"
    ...
    def get_buy_statuses(self) -> list:
        all_statuses = []
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total buy statuses fetched: {len(all_statuses)}")
        return all_statuses
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download buy statuses from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="buy_status.csv", help="Path to output CSV file (default: buy_status.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = BuyStatusFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        statuses = fetcher.get_buy_statuses()
        if statuses:
            writer.save_checkpoint(statuses)
            writer.write(statuses)
        else:
            logger.warning("No buy statuses retrieved.")
    except Exception as e:
