...
def setup_logger(log_filename: str = "justification.log") -> logging.Logger:
    logger = logging.getLogger("JustificationFetcher")
...
class JustificationFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_justification"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download justifications from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="justification.csv", help="Path to output CSV file (default: justification.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = JustificationFetcher(token=args.token, logger=logger)
