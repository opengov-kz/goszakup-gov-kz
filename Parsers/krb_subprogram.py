...
def setup_logger(log_filename: str = "krb_subprogram_parser.log") -> logging.Logger:
    logger = logging.getLogger("KRBSubprogramParser")
...
class KRBSubprogramFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_fkrb_subprogram"
    ...
    def get_subprograms(self) -> list:
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total subprograms fetched: {len(all_subprograms)}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download KRB subprograms from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="krb_subprogram.csv", help="Path to output CSV file (default: krb_subprogram.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = KRBSubprogramFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        subprograms = fetcher.get_subprograms()
        if subprograms:
            writer.save_checkpoint(subprograms)
            writer.write(subprograms)
        else:
            logger.warning("No subprograms retrieved.")
    except Exception as e:
