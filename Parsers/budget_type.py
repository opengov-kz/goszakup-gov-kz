...
def setup_logger(log_filename: str = "budget_type.log") -> logging.Logger:
    logger = logging.getLogger("BudgetTypeFetcher")
...
class BudgetTypeFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_budget_type"
    ...
    def get_budget_types(self) -> list:
        all_budgets = []
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total budget types fetched: {len(all_budgets)}")
        return all_budgets
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download budget types from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="budget_type.csv", help="Path to output CSV file (default: budget_type.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = BudgetTypeFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        budgets = fetcher.get_budget_types()
        if budgets:
            writer.save_checkpoint(budgets)
            writer.write(budgets)
        else:
            logger.warning("No budget types retrieved.")
    except Exception as e:
