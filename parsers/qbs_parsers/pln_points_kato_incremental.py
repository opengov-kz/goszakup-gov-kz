# pln_points_kato_incremental.py
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('pln_points_kato_incremental.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class GoszakupPlansKatoIncrementalParser:
    def __init__(self, token: str, output_file: str = "PlnPointsKato.parquet"):
        self.token = token
    ...
    def fetch_data(self, start_date: datetime) -> list:
    ...
    def run(self):
        ...
        try:
            logger.info("Starting incremental PlansKato data collection...")
            start_date = self.load_existing_data()
            batch_count = self.fetch_data(start_date)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Goszakup PlansKato Incremental GraphQL API Parser")
    parser.add_argument(
        "--token",
        required=True,
    ...
    parser.add_argument(
        "--output",
        default="PlnPointsKato.parquet",
        help="Output parquet file name (default: PlnPointsKato.parquet)"
    )
...
def main():
    ...
    parser = GoszakupPlansKatoIncrementalParser(
        token=args.token,
