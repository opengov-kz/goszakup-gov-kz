    ...
    def load_existing_data(self):
        ...
        try:
            if os.path.exists(self.output_file):
                ...
                self.seen_ids = set(self.existing_data["pid"])
    ...
    def fetch_data(self) -> list:
        ...
        while next_page:
            try:
                ...
                new_items = [item for item in items if item["pid"] not in self.seen_ids]
                if not new_items:
                    logger.info("API returned only duplicates, stopping")
                    break

                all_data.extend(new_items)
                self.seen_ids.update(item["pid"] for item in new_items)  # Fixed to use "pid"
                page_count += 1
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Goszakup Plans Incremental API Parser")
    parser.add_argument(
        "--token",
        required=True,
    ...
    parser.add_argument(
        "--output",
        default="v3Plans.parquet",
        help="Output parquet file name (default: v3Plans.parquet)"
    )
...
def main():
    ...
    parser = GoszakupPlansIncrementalParser(
        token=args.token,
