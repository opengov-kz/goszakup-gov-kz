...
def setup_logger(log_filename: str = "currency.log") -> logging.Logger:
    logger = logging.getLogger("CurrencyFetcher")
...
class CurrencyFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_currency"
    ...
    def get_currencies(self) -> list:
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total currencies fetched: {len(all_currencies)}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download currencies from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="currency.csv", help="Path to output CSV file (default: currency.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = CurrencyFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        currencies = fetcher.get_currencies()
        if currencies:
            writer.save_checkpoint(currencies)
