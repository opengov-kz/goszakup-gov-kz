...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('treasury_pay.log'),
...
class TreasuryPayParser:
    def __init__(self, token: str, year: int, output_prefix: str = None):
        self.token = token
        self.year = year
        self.base_prefix = output_prefix if output_prefix else "TreasuryPay"
        ...
        self.treasury_file = os.path.join(self.output_dir, f"{self.base_prefix}_{year}.parquet")
    ...
    def load_existing_min_date(self) -> datetime:
        """Load the minimum contract_crdate from the existing parquet file for the specified year"""
        try:
            min_date = None
                ...
                if not df_existing.empty:
                    ...
                    if not valid_dates.empty:
                        ...
                        if not df_year.empty:
                            ...
                            logger.info(f"Found {len(df_year)} existing records in {self.treasury_file} for {self.year}, min contract_crdate: {min_date}")
                        else:
                            logger.info(f"No existing records for {self.year} in {self.treasury_file}")
                    ...
                    if not valid_dates.empty:
                        ...
                        if not temp_year_df.empty:
                            ...
                            if min_date is None or (temp_min_date is not None and temp_min_date < min_date):
                                ...
                                logger.info(f"Updated min contract_crdate from temp file {temp_file}: {min_date}")

            if min_date is not None:
                return min_date - timedelta(seconds=1)
            else:
                logger.info(f"No valid contract_crdate found for {self.year}. Starting from end of year.")
    ...
    def fetch_data(self) -> tuple[int, int]:
        """Fetch TreasuryPay data for the specified year"""
        ...
        while current_end_date > self.start_date:
            ...
            while True:  # Pagination loop
                ...
                try:
                    ...
                    logger.info(f"Loaded: {count_loaded_contracts} contracts, {count_loaded_treasury} treasury pays for range {current_start_date} to {current_end_date}")
    ...
    def merge_temp_files(self):
        ...
        try:
            ...
            if not temp_treasury_files:
                logger.info(f"No new data to merge for {self.treasury_file}.")
                return 0
    ...
    def run(self):
        ...
        try:
            logger.info(f"Starting TreasuryPay data collection for year {self.year}...")
            ...
            if total_loaded_treasury == 0:
                logger.info(f"No treasury pays found for year {self.year}.")
            else:
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Goszakup TreasuryPay API Parser")
    parser.add_argument(
        "--token",
        required=True,
    ...
    parser.add_argument(
        "--output-prefix",
        help="Custom prefix for output parquet file (default: TreasuryPay)"
    )
