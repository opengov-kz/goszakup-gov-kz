...
def setup_logger(log_filename: str = "ref_reason_parser.log") -> logging.Logger:
    """Configure and return a logger."""
    logger = logging.getLogger("RefReasonParser")
...
class RefReasonFetcher:
    """Class to interact with the Goszakup API and retrieve reasons for RNU."""

    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_reason"
    ...
    def get_ref_reasons(self) -> list:
        """Fetch reasons from the API."""
        self.logger.info("Sending request to fetch reasons...")
        try:
...
def parse_args():
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Download reasons from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="ref_reason.csv", help="Path to output CSV file (default: ref_reason.csv)")
...
def main():
    ...
    fetcher = RefReasonFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        reasons = fetcher.get_ref_reasons()

        if reasons:
            writer.save_checkpoint(reasons)
            writer.write(reasons)
        else:
            logger.warning("No reasons retrieved.")
