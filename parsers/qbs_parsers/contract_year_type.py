...
def setup_logger(log_filename: str = "contract_year_type.log") -> logging.Logger:
    logger = logging.getLogger("ContractYearTypeFetcher")
...
class ContractYearTypeFetcher:
    BASE_URL = "https://ows.goszakup.gov.kz/v3/refs/ref_contract_year_type"
    ...
    def get_contract_year_types(self) -> list:
        all_types = []
        ...
        while next_page_url:
            ...
            try:
                ...
                raise
        self.logger.info(f"Total contract year types fetched: {len(all_types)}")
        return all_types
...
def parse_args():
    parser = argparse.ArgumentParser(description="Download contract year types from Goszakup API and save to CSV.")
    parser.add_argument("--token", required=True, help="Access token for Goszakup API")
    parser.add_argument("--output", default="contract_year_type.csv", help="Path to output CSV file (default: contract_year_type.csv)")
    return parser.parse_args()

def main():
    args = parse_args()
    logger = setup_logger()
    fetcher = ContractYearTypeFetcher(token=args.token, logger=logger)
    writer = CSVWriter(output_path=args.output, logger=logger)

    try:
        year_types = fetcher.get_contract_year_types()
        if year_types:
            writer.save_checkpoint(year_types)
            writer.write(year_types)
        else:
            logger.warning("No contract year types retrieved.")
    except Exception as e:
